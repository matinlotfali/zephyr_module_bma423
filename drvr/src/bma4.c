 ...
 *  @return None *
 */
static void convert_lsb_g( const struct selftest_delta_limit *accel_data_diff, struct selftest_delta_limit *accel_data_diff_mg,
 ...
 *  sensor.
 */
uint16_t bma4_second_if_mag_compensate_xyz( struct bma4_mag_fifo_data mag_fifo_data, uint8_t mag_second_if, struct bma4_mag *compensated_mag_data )
{
    uint16_t rslt = 0;
#ifdef BMM150
    int16_t mag_x = 0;
...
uint16_t bma4_set_int_pin_config( const struct bma4_int_pin_config *int_pin_config, uint8_t int_line, struct bma4_dev *dev )
{
    ...
    if( dev == NULL ) {
        ...
        if( int_line <= 1 ) {
            data = ( (uint8_t)( ( int_pin_config->edge_ctrl & BMA4_INT_EDGE_CTRL_MASK ) | ( ( int_pin_config->lvl << 1 ) & BMA4_INT_LEVEL_MASK ) |
...
static void check_empty_fifo( uint16_t *data_index, const struct bma4_dev *dev )
{
    if( ( *data_index + 2 ) < dev->fifo->length ) {
        ...
        if( ( dev->fifo->data[*data_index] == FIFO_MSB_CONFIG_CHECK ) && ( dev->fifo->data[*data_index + 1] == FIFO_LSB_CONFIG_CHECK ) ) {
            /*Update the data index as complete*/
...
static uint16_t validate_selftest( const struct selftest_delta_limit *accel_data_diff, const struct bma4_dev *dev )
{
     ...
     * 400mg */
        if( ( accel_data_diff->x > BMA42X_ST_ACC_X_AXIS_SIGNAL_DIFF ) && ( accel_data_diff->y > BMA42X_ST_ACC_Y_AXIS_SIGNAL_DIFF ) &&
     ...
     * 1800mg */
        if( ( accel_data_diff->x > BMA45X_ST_ACC_X_AXIS_SIGNAL_DIFF ) && ( accel_data_diff->y > BMA45X_ST_ACC_Y_AXIS_SIGNAL_DIFF ) &&
